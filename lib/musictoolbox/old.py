...
def fixopen(*args, **kwargs):
    ...
    if filename.lower().endswith(".flac"):
        return mutagen.flac.Open(*args, **kwargs)
    else:
...
def convert_gain_from_mp3gain_to_txxx(gain):
    ...
    try:
        ...
        raise ValueError("invalid gain value %r" % gain)
    return "%.2f dB" % gain
...
REPLAYGAIN_TAGS = (
    (
        ...
        convert_gain_from_mp3gain_to_txxx,
        convert_gain_from_txxx_to_mp3gain,
    ),
...
def write_soundcheck_tag(filename, text):
    ...
    metadata["COMM:iTunNORM:'eng'"] = mutagen.id3.COMM(
        encoding=3, lang="eng", desc="iTunNORM", text=[str(text)]
    )
    metadata.save()
...
def recalc_soundcheck(filename, mp3gain_values):
    ...
    if type is MP3GAIN_ALBUM:
        values = [ag, tg]
    elif type is MP3GAIN_TRACK:
        values = [tg, ag]
...
def txxx_to_mp3gain(filename):
    i = mutagen.id3.ID3(filename)
    try:
        o = mutagen.apev2.APEv2(filename)
    except mutagen.apev2.APENoHeaderError:
        o = mutagen.apev2.APEv2()
    changed = False
    for name, f, g in REPLAYGAIN_TAGS:
        try:
            o[name.upper()] = g(str(i["TXXX:%s" % name]))
            changed = True
        except KeyError:
            pass
    if changed:
...
def apply_mp3gain(filename, apply=False):
    ...
    try:
        tg, tp, ag, ap = get_mp3gain_tags(filename)
    except TypeError:
        tg, tp, ag, ap = [None, None, None, None]
    cmd = ["mp3gain", "-T", "-f"]
    if apply is MP3GAIN_ALBUM:
        if ag:
            ...
            cmd.extend(["-r", "-c"])
    elif apply is MP3GAIN_TRACK:
        cmd.extend(["-r", "-c"])
    else:
        pass
    cmd = cmd + [filename]
    if len(cmd) == 4:
        debug("Computing mp3gain ReplayGain with %s", " ".join(cmd))
    else:
        debug("Applying mp3gain ReplayGain with %s", " ".join(cmd))
...
def mediafilesindir(directory):
    newfiles = []
    for globspec in [
        "*%s" % k for k in list(calc_commands.keys()) if not k.endswith("1")
    ]:
...
def getalbum(filename):
    ...
    if not list(metadata.keys()):
        return None

    try:
        album = str(metadata["TALB"]).lower()
    except KeyError:
        try:
            album = str(metadata["album"]).lower()  # if keyerror, no tiene album
        except KeyError:
            album = ""
    # just in case
    if not album.strip():
...
def alreadyhasreplay(f, album=False):
    try:
        if f.lower().endswith(".mp3"):
            metadata = mutagen.apev2.Open(f)
        else:
            metadata = fixopen(f)
    except (mutagen.apev2.APENoHeaderError, KeyError):
        return False
    if album:
        if "replaygain_album_gain" in metadata:
            return True
        if "REPLAYGAIN_ALBUM_GAIN" in metadata:
            return True
    else:
        if "replaygain_track_gain" in metadata:
            return True
        if "REPLAYGAIN_TRACK_GAIN" in metadata:
            return True
    return False
...
def alreadyhasrva2(f, album=False):
    try:
        metadata = mutagen.id3.Open(f)
    except mutagen.id3.ID3NoHeaderError:
        return False
    for value in list(metadata.values()):
        try:
            if album and value.desc == "replaygain_album_gain":
                return True
            elif value.desc == "replaygain_track_gain":
                return True
        except AttributeError:
            continue
    return False
...
def doreplaygain(args, options):
    ...
    while filenames:
        ...
        if album:
            ...
            if not options.quiet:
                log(
                    "Found %s new files from the same album in the directory",
                    (len(files) - l),
                )
            ...
            if not options.quiet:
                log("Found %s new files from command line arguments", (len(files) - l))
        ...
        for extension, files in list(groups.items()):
            ...
            if (
                ...
                and (
                    "mp3" not in extension
                    or all([alreadyhasrva2(f, album=album) for f in files])
                )
            ):

                if not options.quiet:
                    log(
                        "All (%s) files already have replaygain (album %s), we skip them",
                        len(files),
                        album,
                    )
                ...
                if withoutape:
                    if not options.quiet:
                        log(
                            "These files do not already have ReplayGain (album %s):"
                            % album
                        )
                    for x in withoutape:
                        if not options.quiet:
                ...
                if "mp3" in extension:
                    withoutrva2 = [
                        x for x in files if not alreadyhasrva2(x, album=album)
                    ]
                    if withoutrva2:
                        if not options.quiet:
                            log(
                                "These files do not already have RVA2 (album %s):"
                                % album
                            )
...
def debug(*args):
    return logging.getLogger("transcoder").debug(*args)


def err(*args):
    return logging.getLogger("transcoder").exception(*args)


def info(*args):
    return logging.getLogger().info(*args)


def warning(*args):
    return logging.getLogger().warn(*args)
...
def maketempcopy(uri):
    ...
    dir = None
    if os.access("/dev/shm", os.W_OK) and not uri.lower().endswith("mpc"):
        dir = "/dev/shm"
...
class NoDecoderException(TranscoderException):
    pass


class NoEncoderException(TranscoderException):
    pass
...
def decode(input, source_format):
    ...
    try:
        subprocess.check_call(
            decoder, stdout=open("/dev/null", "wb"), stderr=subprocess.STDOUT
        )
    except Exception:
...
def detect_encoders():
    ...
    if command_available("lame"):
        encoders[
            "mp3"
        ] = lambda i, o: "lame -q 0 --noreplaygain --nohist --vbr-new".split(" ") + [
            i,
            o,
        ]
...
def encode(input, target_format):
    ...
    try:
        subprocess.check_call(
            encoder, stdout=open("/dev/null", "wb"), stderr=subprocess.STDOUT
        )
    except Exception:
        if os.path.exists(output):
            os.unlink(output)
...
def transfer_tags_any_mp3(origin, destination, tag_reader):
    ...
    try:
        ...
        err("Could not open source file tag: %s" % e)
        return
...
def transfer_tags_mp3(origin, destination):
    ...
    try:
        ...
        err("Though it exists, an error blocked opening the APEv2 tag: %s" % e)
    ...
    try:
        ...
        err("Though it exists, an error blocked opening the ID3 tag: %s" % e)
...
def check_transcodable(source_format, target_format):
    ...
    if type(target_format) in (list, tuple):
        if source_format in target_format:
            needs_transcoding = False
        else:
    ...
    if source_format not in list(decoders.keys()):
        raise NoDecoderException(source_format)
    if type(target_format) in (list, tuple):
        ...
        for t in target_format:
            if t in list(encoders.keys()):
                ...
                break
        if not choice:
            raise NoEncoderException(target_format)
        target_format = choice
    else:
        if target_format not in list(encoders.keys()):
            raise NoEncoderException(target_format)
...
def transcode(uri, target_format):
    ...
    if source_format.startswith("."):
        raise Exception("source format cannot start with a dot")
    if target_format.startswith("."):
        raise Exception("target format cannot start with a dot")
...
def parallel_transcode(format, paths):
    uris = [
        "file://" + urllib.request.pathname2url(os.path.abspath(path)) for path in paths
    ]
...
def relativize(listoffiles, commonprefix=None):
    if not commonprefix:
        commonprefix = os.path.commonprefix(listoffiles)
...
class SyncManager:
    ...
    def source_song_found(self, path, date):
        ...
        if (
            newpath in self.destination_songs
            and date >= self.destination_song_dates[newpath]
        ):
    ...
    def scan_destination(self):
        ...
        def do_scan():
            for base, dirs, files in os.walk(destination):
                if base == self.playlistdir:
                    continue
                for f in files:
                    try:
                        thefilename = os.path.join(base, f)
                        self.destination_song_found(
                            thefilename, os.stat(thefilename).st_mtime
                        )
                    except Exception as e:
                        self.lock.acquire()
                        self.exceptions.append(e)
    ...
    def manifest_transfer(self):
        ...
        srcs_and_dests = [
            (self.source_songs[k], os.path.join(self.destdir, k)) for k in to_transfer
        ]
        ...
        if srcs_and_dests:
            ...
            try:
                ...
                for r in result:
                    i = i + 1
                    log(
                        "%s of %s done (%s %%).  Last result: %r"
                        % (i, total, i * 100 / total, r)
                    )
                pool.close()
                pool.join()
    ...
    def remove_obsolete_songs(self):
        songsthatneedtobedeleted = set(self.destination_songs) - set(
            self.source_songs.keys()
        )
